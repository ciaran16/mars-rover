...
import { COMMANDS, DIRECTIONS, Robot } from "./Robot.js";
export function processInput(inputLines) {
    ...
    const robotsWithCommandsResult = flattenResults(robotInputs.map((robotInput) => parseRobotInput(robotInput)));
    if (!robotsWithCommandsResult.ok) {
        return robotsWithCommandsResult;
    }
    const outputLines = robotsWithCommandsResult.value.map(({ robot, commands }) => {
...
function parseRobotInput(robotInput) {
